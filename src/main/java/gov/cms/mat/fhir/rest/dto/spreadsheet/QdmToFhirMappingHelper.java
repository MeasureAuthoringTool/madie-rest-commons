...
public class QdmToFhirMappingHelper {
  ...
  public QdmToFhirMappingHelper(List<QdmToQicoreMapping> mappings) {
    mappings.stream()
        .filter(
            m ->
                StringUtils.isNotBlank(m.getMatDataType())
                    && StringUtils.isNotBlank(m.getFhirQICoreMapping())
                    && m.getFhirQICoreMapping().contains("."))
        .forEach(
            m -> {
              ...
              if (StringUtils.isNotBlank(qdmAttrib)) {
                ...
                Arrays.stream(qdmAttribs)
                    .forEach(a -> qdmTypeAttributeMappings.put(Pair.of(qdmType, a), m));
              }
            });
  }
